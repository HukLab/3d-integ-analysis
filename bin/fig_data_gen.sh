...
python pcorVsDurByCoh.py -l --fit --subj $MODEL_SUBJ -r $RESAMPLE --outdir $OUTDIR
by dotmode, ALL
echo "SAT_EXP for ALL..."
python pcorVsDurByCoh.py -l --fit -r $RESAMPLE --outdir $OUTDIR
# by coherence, ALL, (2D, 3D)
echo "SAT_EXP for ALL (2D, 3D)..."
python fitCurveVsDurByCoh.py -l --is_agg_subj --fits sat-exp --nboots $NBOOTS_SAT_EXP -r $RESAMPLE --outdir $OUTDIR
