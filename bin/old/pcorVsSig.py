...
def bootstrap_solve(xs0, ys0, nboots, unfold, fit_lapses, thresh_val=0.75):
    guess = (1500, 7) if unfold else (85, 0.75)
    if fit_lapses:
        guess = guess + (0.01, 0.99)
...
def main(args, nboots, unfold, fit_lapses, savefig, outdir):
    df = load(args)
    ...
    for dotmode, dfc in df.groupby('dotmode'):
        ...
        thetas, threshes = bootstrap_solve(xs, ys, nboots, unfold, fit_lapses)
    ...
    if unfold:
        ...
        plt.ylim([0.45, 1.05])
    plt.xlim([None, None])
    subj_label = subjs[0] if len(subjs) == 1 else 'ALL'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nboots', type=int, default=0)
    parser.add_argument('--unfold', action='store_true', default=False)
    parser.add_argument('--fit-lapses', action='store_true', default=False)
    parser.add_argument('--savefig', action='store_true', default=False)
    parser.add_argument('--outdir', type=str, default='.')
    args = parser.parse_args()
    main({'subj': args.subj, 'dotmode': args.dotmode}, args.nboots, args.unfold, args.fit_lapses, args.savefig, args.outdir)
